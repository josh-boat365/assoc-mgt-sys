<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('user.home') }}">
                        <img class="ams-logo-dash" src="{{ asset('imgs/ams-logo-short.png') }}" alt="AMS Logo">
                    </a>
                </div>
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            ...
                            @empty(Auth::user()->firstname && Auth::user()->surname)
                             <div class="mr-3 font-semibold">{{ Auth::user()->username }}</div>
                             @else
                                <div class="mr-3 font-semibold">{{ Auth::user()->firstname }} {{ Auth::user()->surname }}</div>
                            @endempty
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('user.home')" :active="request()->routeIs('dashboard')">
                {{ __('Dashboard') }}
